...
export const fireMissionService = {
    ...
    assignArtillery: async (id: string, artilleryId: string, projectileType?: string, charge?: number): Promise<FireMission> => {
        const response = await apiClient.fetch(`${API_URL}/${id}/assign`, {
            method: 'PUT',
            body: JSON.stringify({ artilleryId, projectileType, charge: charge?.toString() }),
            headers: { 'Content-Type': 'application/json' }
        });
        return await response.json();
    }
