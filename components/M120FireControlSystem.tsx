...
import { AVAILABLE_PROJECTILES_M120, calculateM120Solution } from '../utils/ballistics';
...
interface M120FireControlSystemProps {
  isOpen: boolean;
  onClose: () => void;
...
export const M120FireControlSystem: React.FC<M120FireControlSystemProps> = ({
  isOpen,
  onClose,
  pendingMission,
  artilleryPieces,
  ...
  const [selectedProjectile, setSelectedProjectile] = useState<ProjectileType>("M931 HE");
  ...
  useEffect(() => {
    const availableCharges = AVAILABLE_PROJECTILES_M120[selectedProjectile as keyof typeof AVAILABLE_PROJECTILES_M120]?.charges || [];
  ...
  const handleCalculate = () => {
    ...
    const { solution, trajectory } = calculateM120Solution(
    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-80 flex items-center justify-center z-[2000] p-4 backdrop-blur-sm">
      <div className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col border border-gray-700">
        <header className="flex justify-between items-center p-4 border-b border-gray-700">
          <h2 className="text-xl font-semibold text-amber-300">Procesar Misión - Mortero 120mm M120</h2>
        ...
        <main className="flex-1 flex overflow-hidden p-4 gap-4">
          <section className="w-1/4 flex flex-col gap-4">
            ...
            <div className="flex-1 flex flex-col bg-gray-900/50 p-3 rounded-md">
                <h3 className="font-semibold text-gray-300 border-b border-gray-700 pb-2 mb-2">Morteros Asignados</h3>
          ...
          <section className="w-2/4 flex flex-col gap-4">
              <div className="bg-gray-900/50 p-3 rounded-md">
                  <h3 className="font-semibold text-gray-300 border-b border-gray-700 pb-2 mb-2">Configuración de Tiro (M120)</h3>
                  <div className="grid grid-cols-2 gap-4">
                      <div>
                          ...
                          <select value={selectedProjectile} onChange={e => setSelectedProjectile(e.target.value as ProjectileType)} disabled={!selectedArtillery} className="w-full p-2 bg-gray-700 border-gray-600 rounded mt-1 disabled:opacity-50">
                                {Object.keys(AVAILABLE_PROJECTILES_M120).map(p => <option key={p} value={p}>{p}</option>)}
                          </select>
                      </div>
                      <div>
                          <label className="text-sm text-gray-400 block">Carga</label>
                           <select value={selectedCharge} onChange={e => setSelectedCharge(Number(e.target.value))} disabled={!selectedArtillery || AVAILABLE_PROJECTILES_M120[selectedProjectile as keyof typeof AVAILABLE_PROJECTILES_M120]?.charges.length === 0} className="w-full p-2 bg-gray-700 border-gray-600 rounded mt-1 disabled:opacity-50">
                                {AVAILABLE_PROJECTILES_M120[selectedProjectile as keyof typeof AVAILABLE_PROJECTILES_M120]?.charges.map(c => <option key={c} value={c}>{c}</option>)}
                           </select>
                      </div>
                      <div>
                   ...
                   <button onClick={handleCalculate} disabled={!selectedArtilleryId} className="w-full mt-4 p-2 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded flex items-center justify-center disabled:opacity-50">
                      <ChartBarIcon className="w-5 h-5 mr-2" />
                      Calcular Solución de Tiro (M120)
                   </button>
              </div>
