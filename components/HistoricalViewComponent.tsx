...
import { useAITask } from '../utils/geminiService';
...
export const HistoricalViewComponent: React.FC<HistoricalViewProps> = ({ 
  ...
  const q5Task = useAITask('q5Generation');
  const q5ExistsForSelectedAAR = selectedAAR && q5Reports.some(q5 => q5.aarId === selectedAAR.id);
  const isGeneratingQ5ForSelectedAAR = selectedAAR && (q5GeneratingStatus[selectedAAR.id] || q5Task.status === 'QUEUED' || q5Task.status === 'RUNNING');
            ...
            <div className="space-y-3 text-gray-200">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                ...
                {!q5ExistsForSelectedAAR && (
                  ...
                  >
                    {q5Task.status === 'QUEUED' ? (
                      <>
                        <ArrowPathIcon className="w-4 h-4 mr-1.5 animate-spin text-amber-400" />
                        <span className="text-amber-400">Cola: #{q5Task.queuePosition}...</span>
                      </>
                    ) : q5Task.status === 'RUNNING' || q5GeneratingStatus[selectedAAR.id] ? (
                      <>
