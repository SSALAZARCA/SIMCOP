...
interface TelegramConfigProps {
  ...
  onSendTest: (chatId: string, botType?: 'ARTILLERY' | 'COMMS') => Promise<boolean>;
...
export const TelegramConfigComponent: React.FC<TelegramConfigProps> = ({
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-lg shadow-inner">
      ...
      <div className="bg-gray-750 p-4 rounded-md mb-6 border border-gray-600">
        <h4 className="text-md font-semibold text-blue-300 mb-2">Token del Bot de Artillería (HTTP API)</h4>
        <p className="text-xs text-gray-400 mb-3">
          Configure el token global del bot que enviará los mensajes. Si el sistema ya tiene uno configurado, los envíos de prueba deberían funcionar.
        </p>
        <div className="flex gap-2">
          <input
            type="password"
            placeholder="Ej: 123456789:ABCdefGHIjklMNOpqrsTUVwxyz..."
            className="flex-grow bg-gray-700 p-2 rounded-md text-sm border border-gray-600 focus:border-blue-500 outline-none text-white"
            id="global-telegram-token"
          />
          <button
            onClick={async () => {
              const input = document.getElementById('global-telegram-token') as HTMLInputElement;
              ...
              try {
                ...
                alert('Error al guardar el token: ' + e.message);
              }
            }}
      ...
      <h4 className="text-md font-semibold text-gray-300 mb-3">Chat IDs por Usuario</h4>
