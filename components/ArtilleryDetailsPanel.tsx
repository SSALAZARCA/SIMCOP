...
import { calculateDistanceAndAzimuth, calculateM101A1Solution, calculateLG1Solution, calculateL119Solution, calculateM120Solution, calculateHY112Solution } from '../utils/ballistics';
...
export const ArtilleryDetailsPanel: React.FC<ArtilleryDetailsPanelProps> = ({ piece, allUsers, activeFireMissions, confirmShotFired, currentUser, deleteArtilleryPiece }) => {
  ...
  const ballisticData = useMemo(() => {
      if (!activeMission) return null;
      
      const { distance, azimuth } = calculateDistanceAndAzimuth(piece.location, activeMission.target);
      let elevation = 0;
      let flightTime = 0;

      const proj = activeMission.projectileType as any;
      const chg = activeMission.charge;
      
      try {
        if (piece.type.includes('M101A1')) {
           const { solution } = calculateM101A1Solution(piece.location, activeMission.target, 0, 0, proj, chg);
           if (solution) { elevation = solution.elevation; flightTime = solution.flightTime; }
        } else if (piece.type.includes('LG1')) {
           const { solution } = calculateLG1Solution(piece.location, activeMission.target, 0, 0, proj, chg, 0, 15);
           if (solution) { elevation = solution.elevation; flightTime = solution.flightTime; }
        } else if (piece.type.includes('L119')) {
           const { solution } = calculateL119Solution(piece.location, activeMission.target, 0, 0, proj, chg, 0, 15, 1013);
           if (solution) { elevation = solution.elevation; flightTime = solution.flightTime; }
        } else if (piece.type.includes('M120')) {
           const { solution } = calculateM120Solution(piece.location, activeMission.target, 0, 0, proj, chg, 0, 15);
           if (solution) { elevation = solution.elevation; flightTime = solution.flightTime; }
        } else if (piece.type.includes('HY112')) {
           const { solution } = calculateHY112Solution(piece.location, activeMission.target, 0, 0, proj, chg, 0, 15);
           if (solution) { elevation = solution.elevation; flightTime = solution.flightTime; }
        }
      } catch (e) {
          console.error("Error calculating local solution", e);
      }
      
      return { distance, azimuth, elevation, flightTime };
  }, [activeMission, piece.location, piece.type]);
    ...
    <div className="space-y-4 text-gray-200">
      ...
      {activeMission && (
        <div className="bg-red-900/50 border-2 border-red-500 rounded-lg p-3 space-y-2 animate-pulse">
          ...
          <div className="text-xs grid grid-cols-2 gap-1 bg-gray-900/50 p-2 rounded">
            <p><span className="font-semibold text-gray-400">Blanco:</span> {decimalToDMS(activeMission.target)}</p>
            <p><span className="font-semibold text-gray-400">Distancia:</span> {ballisticData ? (ballisticData.distance / 1000).toFixed(2) + ' km' : '---'}</p>
            <p><span className="font-semibold text-gray-400">Azimut:</span> {ballisticData ? ballisticData.azimuth.toFixed(2) + '°' : '---'}</p>
            <p><span className="font-semibold text-gray-400">Elevación:</span> {ballisticData?.elevation ? ballisticData.elevation.toFixed(2) + '°' : '---'}</p>
            ...
            <p><span className="font-semibold text-gray-400">T. Vuelo:</span> {ballisticData?.flightTime ? ballisticData.flightTime.toFixed(1) + ' s' : '---'}</p>
