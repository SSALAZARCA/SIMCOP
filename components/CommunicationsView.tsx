...
export const CommunicationsView: React.FC<CommunicationsViewProps> = ({
  ...
  const handleTestNotification = async () => {
    ...
    console.log("Iniciando prueba de notificación Telegram (COMUNICACIONES)...");
    try {
      const success = await sendTestTelegramAlert(currentUserTelegramChatId, 'COMMS'); // Call with ID and botType
      if (success) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-4">
          <div className="bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            {currentUser?.role === 'ADMINISTRATOR' && (
              <div className="mb-4 p-3 bg-gray-750 border border-gray-600 rounded">
                <label className="block text-sm font-medium text-blue-300 mb-1">
                  Token del Bot de Comunicaciones (HTTP API)
                </label>
                <p className="text-xs text-gray-400 mb-2">Configure el token global del bot.</p>
                <div className="flex gap-2">
                  <input
                    type="password"
                    id="global-bot-token"
                    placeholder="Ej: 123456789:ABCdefGHI..."
                    className="flex-grow px-3 py-1 bg-gray-700 border border-gray-600 rounded-md text-white text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                  <button
                    onClick={async () => {
                      const input = document.getElementById('global-bot-token') as HTMLInputElement;
