...
import { getPredictiveLogisticsAnalysis, useAITask, shouldTriggerAutoAI } from '../utils/geminiService';
...
const PredictiveLogisticsComponent: React.FC<PredictiveLogisticsProps> = ({ units, addLogisticsRequest }) => {
  const taskState = useAITask('predictiveLogistics');

  // Keep latest units in ref to avoid re-triggering the useEffect interval on every telemetry update
  ...
  const fetchPredictions = useCallback(async (force = false) => {
    if (!force && !shouldTriggerAutoAI('predictiveLogistics')) {
      return;
    }
    try {
      await getPredictiveLogisticsAnalysis(unitsRef.current, force);
    } catch (err: any) {
      console.error("Error fetching predictive logistics:", err);
    }
  }, []); // Stable dependencies

  useEffect(() => {
    fetchPredictions(false); // Check strategic hours on mount
    const intervalId = setInterval(() => fetchPredictions(false), 60000); // Check every 60 seconds
  ...
  return (
    <div className="bg-gray-800 p-2 md:p-4 rounded-lg shadow-inner">
      <h3 className="text-lg font-semibold text-gray-300 mb-3 border-b border-gray-700 pb-2 flex justify-between items-center">
        ...
        <button
          ...
          disabled={isLoading}
          className="text-xs bg-yellow-500/20 hover:bg-yellow-500/40 disabled:opacity-50 text-yellow-200 px-3 py-1 rounded border border-yellow-500/30 transition-all font-bold uppercase tracking-wider flex items-center gap-1"
        >
      ...
      {taskState.status === 'QUEUED' ? (
        ...
        <div className="flex flex-col items-center justify-center py-8 text-blue-400">
          ...
          <span className="text-xs font-semibold uppercase tracking-widest">
            Procesando predicciones logísticas en IA...
          </span>
        </div>
      ) : taskState.status === 'FAILED' ? (
        <div className="text-center text-red-400 py-6">{taskState.error || 'Fallo al obtener análisis logístico.'}</div>
