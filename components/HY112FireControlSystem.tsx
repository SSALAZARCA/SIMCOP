...
import { AVAILABLE_PROJECTILES_HY112, calculateHY112Solution } from '../utils/ballistics';
...
interface HY112FireControlSystemProps {
  isOpen: boolean;
  onClose: () => void;
...
export const HY112FireControlSystem: React.FC<HY112FireControlSystemProps> = ({
  isOpen,
  onClose,
  pendingMission,
  artilleryPieces,
  ...
  const [selectedProjectile, setSelectedProjectile] = useState<ProjectileType>("HE Y12-HE");
  const [selectedCharge, setSelectedCharge] = useState<number>(3);
  ...
  useEffect(() => {
    const availableCharges = AVAILABLE_PROJECTILES_HY112[selectedProjectile as keyof typeof AVAILABLE_PROJECTILES_HY112]?.charges || [];
  ...
  const handleCalculate = () => {
    ...
    const { solution, trajectory } = calculateHY112Solution(
        selectedArtillery.location,
        pendingMission.target,
        600, 650, // Placeholder altitudes
        selectedProjectile,
        selectedCharge,
        wind,
        temperature
    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-80 flex items-center justify-center z-[2000] p-4 backdrop-blur-sm">
      <div className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col border border-gray-700">
        <header className="flex justify-between items-center p-4 border-b border-gray-700">
          <h2 className="text-xl font-semibold text-teal-300">Procesar Misión - Mortero 120mm HY1-12</h2>
        ...
        <main className="flex-1 flex overflow-hidden p-4 gap-4">
          <section className="w-1/4 flex flex-col gap-4">
            ...
            <div className="flex-1 flex flex-col bg-gray-900/50 p-3 rounded-md">
                <h3 className="font-semibold text-gray-300 border-b border-gray-700 pb-2 mb-2">Morteros HY1-12 Asignados</h3>
          ...
          <section className="w-2/4 flex flex-col gap-4">
              <div className="bg-gray-900/50 p-3 rounded-md">
                  <h3 className="font-semibold text-gray-300 border-b border-gray-700 pb-2 mb-2">Configuración de Tiro (HY1-12)</h3>
                  <div className="grid grid-cols-2 gap-4">
                      <div>
                          ...
                          <select value={selectedProjectile} onChange={e => setSelectedProjectile(e.target.value as ProjectileType)} disabled={!selectedArtillery} className="w-full p-2 bg-gray-700 border-gray-600 rounded mt-1 disabled:opacity-50">
                                {Object.keys(AVAILABLE_PROJECTILES_HY112).map(p => <option key={p} value={p}>{p}</option>)}
                          </select>
                      </div>
                      <div>
                          <label className="text-sm text-gray-400 block">Carga</label>
                           <select value={selectedCharge} onChange={e => setSelectedCharge(Number(e.target.value))} disabled={!selectedArtillery || AVAILABLE_PROJECTILES_HY112[selectedProjectile as keyof typeof AVAILABLE_PROJECTILES_HY112]?.charges.length === 0} className="w-full p-2 bg-gray-700 border-gray-600 rounded mt-1 disabled:opacity-50">
                                {AVAILABLE_PROJECTILES_HY112[selectedProjectile as keyof typeof AVAILABLE_PROJECTILES_HY112]?.charges.map(c => <option key={c} value={c}>{c}</option>)}
                           </select>
                      </div>
                      <div>
                   ...
                   <button onClick={handleCalculate} disabled={!selectedArtilleryId} className="w-full mt-4 p-2 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded flex items-center justify-center disabled:opacity-50">
                      <ChartBarIcon className="w-5 h-5 mr-2" />
                      Calcular Solución de Tiro (HY1-12)
                   </button>
              </div>
