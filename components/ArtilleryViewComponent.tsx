...
interface ArtilleryViewProps {
  ...
  sendTestTelegramAlert: (chatId?: string, botType?: 'ARTILLERY' | 'COMMS') => Promise<boolean>;
...
export const ArtilleryViewComponent: React.FC<ArtilleryViewProps> = ({
  ...
  const isCdtUser = currentUser?.role === UserRole.ADMINISTRATOR || !!currentUser?.role.startsWith('DIRECTOR_TIRO');
  const [activeTab, setActiveTab] = useState<ActiveTab>(isCdtUser ? 'cdt' : 'pieces');
  ...
  const filteredFireControlSystems = useMemo(() => {
    if (!currentUser || !currentUser.role.startsWith('DIRECTOR_TIRO')) {
  ...
  const userPendingMissions = useMemo(() => {
    ...
    return pendingFireMissions.filter(mission => {
      ...
      if (mission.status === 'approved') return false; // Hide assigned missions from CDT since they are already sent to the piece
      
      const piece = mission.assignedArtilleryId ? artilleryPieces.find(p => p.id === mission.assignedArtilleryId) : null;
      
      if (!piece) {
          // If it's unassigned, Directors of Tiro and Admins should see it so they can assign it!
          return isHighCommandOrAdmin || currentUser.role.startsWith('DIRECTOR_TIRO');
      }

      if (isHighCommandOrAdmin) return true;

      if (currentUser.role.startsWith('DIRECTOR_TIRO')) {
  ...
  const handleLaunchCdt = (mission: PendingFireMission) => {
    ...
    if (mission.assignedArtilleryId) {
        piece = artilleryPieces.find(p => p.id === mission.assignedArtilleryId);
        if (piece) systemType = piece.type;
    }

    if (!systemType) {
        // If unassigned, default to the FDO's primary system
        if (filteredFireControlSystems.length > 0) {
            systemType = filteredFireControlSystems[0].type;
        } else {
            alert("Error: No tiene sistemas de artillería asignados para procesar esta misión.");
            return;
        }
    }

    setCdtModalState({ type: systemType, mission: mission });
  };
    ...
    <>
      <div className="flex flex-col space-y-4">
        ...
        <div className="flex items-end space-x-1 border-b border-gray-700">
          ...
          {(!currentUser?.role.startsWith('DIRECTOR_TIRO') || currentUser?.role === UserRole.ADMINISTRATOR) && (
            <>
