...
export const FireMissionControlComponent: React.FC<FireMissionControlProps> = ({
  ...
  const renderContent = () => {
    ...
    if (myPendingMission) {
      ...
      if (myPendingMission.status === 'approved') {
        return (
          <div className="text-center p-3 bg-green-900/50 border border-green-600 rounded-md">
              <div className="flex items-center justify-center text-green-400 mb-2">
                <svg className="w-6 h-6 mr-2 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <p className="font-bold text-lg">¡Fuego Autorizado!</p>
              </div>
            <p className="text-xs text-green-300">El CDT ha procesado la misión y enviado los datos de tiro a la pieza asignada. Esperando confirmación de disparo.</p>
          </div>
        );
      }
