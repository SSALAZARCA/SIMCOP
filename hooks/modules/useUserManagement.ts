...
export const useUserManagement = (
    ...
    const sendTestTelegramAlert = useCallback(async (chatId?: string, botType: 'ARTILLERY' | 'COMMS' = 'ARTILLERY') => {
        if (!chatId) {
            setAlertsInternal(prev => [{
                id: generateRandomId(),
        ...
        try {
            const success = await userService.sendTestTelegramMessage(chatId, botType);
            if (success) {
                setAlertsInternal(prev => [{
                    id: generateRandomId(),
                    type: AlertType.INFO,
