    ...
    public ResponseEntity<java.util.Map<String, Object>> getUnitById(@PathVariable String id) {
        return repository.findById(id)
                .map(unit -> {
                    ...
                    // FUSIÓN DE DATOS: Obtener datos reales de SIGEP
                    java.util.Map<String, Object> sigepData = sigepIntegrationService.getRealPersonnelStatus(id);
                    if (sigepData != null) {
                        response.put("sigep_real_status", sigepData);
                    }
